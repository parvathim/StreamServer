...
FeedManager.prototype = {
	...
	activityCreated: function(instance) {
		if (this.trackingEnabled(instance)) {
			...
			var feed = this.getFeed(feedType, activity.actor);

			var promise = feed.subscribe(function(data){
				console.log("feed was updated", data);
 
				//client.on('connect', function () {
				//  client.subscribe('presence')
				data.new.forEach(function(datafeed,idx){
				  client1.publish('newfeed', data.new[idx].object);
				});
				//})
 
			});

			return promise.then(function(){feed.addActivity(activity)});;
		}
	},
